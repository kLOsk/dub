...
struct TrackOverviewView: View {
    ...
    private func drawBars(ctx: GraphicsContext, size: CGSize, buckets: [OverviewBucket]) {
        ...
        // Deck-tinted, and now the *only* place the tint is spent
        // besides the deck's own label. Controls all light in one
        // accent, so this map is what tells the two decks apart —
        // which suits it: it is the per-deck object you look at to
        // find your place, and it is never mistaken for a control.
