...
struct AboutSheet: View {
    ...
    /// The bundled attribution file, generated by `make attribution`.
    ///
    /// Reproducing each dependency's licence text is an obligation of
    /// the MIT / Apache / BSD licences Dub's graph is built from, and
    /// this is where a user reaches it. `nil` if the resource is
    /// missing rather than crashing — a debug build someone assembled
    /// without running `make attribution` should still open its About
    /// panel.
    private var acknowledgmentsURL: URL? {
        Bundle.main.url(forResource: "Acknowledgments", withExtension: "html")
    }
    ...
    private var content: some View {
        VStack(alignment: .leading, spacing: DubSpacing.lg) {
            ...
            HStack(spacing: DubSpacing.md) {
                ...
                if let acknowledgmentsURL {
                    linkButton("Open source licenses", url: acknowledgmentsURL)
                }
            }
        }
