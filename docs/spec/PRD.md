...
- As a Thru deck plays, the engine continuously computes a rolling **audio fingerprint** over a 5-second window. Algorithm: Chromaprint (TEST1 preset — the same preset the stored library fingerprints use; see §10.2), via the pure-Rust `rusty-chromaprint` crate. M11b chose pure-Rust over the LGPL-2.1 C library for the reasons documented in §10.2 (license isolation, no C build dep, no unsafe FFI surface).
...
- **FLAC, not MP3.** Lossless, already in the §4.4 decode set, and encodable in pure Rust under a permissive license (`flacenc`, Apache-2.0). An MP3-320 export would have made LAME (LGPL) the first non-permissive linked library; evaluated and deliberately deferred — see `LICENSE-DEPENDENCIES.md`.
- **Fully offline by design.** Recognition (M26c: AcoustID + MusicBrainz + Discogs, Dub's first network dependency, confined to a `dub-recognize` leaf crate behind an `Http` trait) is optional enrichment — rips never depend on it.

**Status.** M26a is implemented end to end: the Rust core (a stereo pre-gain record tap on the Thru path — underrun zero-fill included, so a dropout rips as honest silence; a crash-safe WAV spill + atomic `rip.json` manifest with idempotent commit retry, artifacts documented in `LIBRARY-SCHEMA.md`; manual split plan; encode + tag + import + side archive), the `dub rip` CLI, and the Prep-mode Swift UI (the Preferences-gated PREP/PERF override switch, the RIP VINYL bar with live level + growing overview waveform, the review screen with draggable/nudgeable split markers — labelled with the track name that starts at each, and mirrored into the playing strip so the split can be judged against the waveform's detail rather than only the overview band — a transport per card — one button that plays *that whole track* and turns into a pause while the playhead is inside it, so at most one card ever shows pause; Space is the same action on whichever track the playhead is in, and there is no panel-level transport because a second Play only raises the question of what it plays that the card's does not — no BPM or KEY on the Prep header while a rip is capturing or in review — deck A holds the whole side as one temporary track, and each track is measured on its own audio at encode, so the only tempo control that matters before encoding is the card's Genre, which picks the analyser's octave — a **bin** on each card to leave a segment out of the commit (the lead-in chatter, a run-out the detector took for music, a track you do not want: the audio stays in `side.flac`, the indices do not shift, and a re-split brings it back — removing a *boundary* instead merges two segments, and both edits are needed), editable metadata cards, and per-segment encode/import progress with idempotent retry). Debug builds additionally allow recording from the built-in soundcard when the Developer mode override is engaged (mono mic duplicated to both channels; raw pre-classifier device list) — a dogfood affordance that never ships in Release. **M26b is implemented end to end** on top of that: **auto gap detection** over the capture envelope (adaptive — the noise floor is measured from the side itself off the 20th-quietest 50 ms cell of the needle-down span, and the gap line is floor + 21 dB, the centre of the plateau three real sides agree on; a side whose music sits under 24 dB above its own floor is *refused* rather than split, which is what keeps a dub breakdown intact when a side holds no true silence), **needle-drop auto-start** (a pre-roll ring holds the last second while armed and is flushed ahead of the trigger, so the drop that started the rip is in the rip), **run-out auto-stop** (relative, not absolute: run-out noise on a played-out 45 sits above any fixed gate worth setting, so the gate measures distance from this side's loudest moment — 30 s at 18 dB down, on RMS rather than peak, because a run-out groove is made of the clicks peak reports), **side trim** (the side is bounded at *both* ends — the last sustained music for the tail, and for the head the first sustained *peak* onset, since a first track fades in out of the lead-in groove and its median sits at groove level long after the music is audible; a needle drop is rejected by requiring the onset to hold. The lead-in and run-out grooves are dropped from the first and last tracks, and the full capture survives in `side.flac`, so a re-split reaches back past both trims), **session recovery** (an interrupted rip is found on disk and reopened at review with its envelope rebuilt from the spill; the length comes from the file, because a crash leaves the manifest claiming zero, and the WAV header is salvaged because `hound` writes the RIFF sizes only at finalize), **live per-segment commit progress**, and **re-split from the lossless archive** (Replace semantics, ordered: the new segments import first and the earlier split's tracks leave the library only once that succeeds — a failure leaves the old split intact rather than neither). `dub rip-tune` reports what every gate would do to a recorded side, so the thresholds are tuned against real recordings rather than against the turntable — and every gate above is fitted to three of them (a reggae 12", a quiet-mastered soul sampler and a drum'n'bass 45), each pinned as a named regression test. The first round was fitted to one side and all three of its numbers were wrong on the next two. Re-split is reachable from the app: the library's **Real Records** node lists committed rips and reopens one into the same review panel, and the review overlay shades the discarded lead-in and run-out with a draggable bracket on each bound (`UI-BACKLOG.md` §6). **M26c (recognition) is in progress**: `dub-recognize` is complete and exercised against the live services through `dub recognize <session-dir | side.wav>`. What a DJ needs off a rip is **artist and title per track**, and a rip is frequently a single song rather than a whole side, so naming is the primary path and costs nothing beyond the fingerprint lookup — AcoustID returns title and artist with the match, and no MusicBrainz call is involved. The transient TEST2 fingerprint (the stored M11b dedupe blobs stay TEST1) goes up in Chromaprint's compressed wire format. Identifying the *pressing* — album, date, label, catalogue number and the A1/B3 track numbers — is **opt-in** (`--album`), and runs a side-level release vote so the tracks of one side agree on one release rather than each guessing alone. The vote is deliberately not allowed to gate naming: measured against a real Philadelphia International sampler, gating it named 2 of 10 tracks while AcoustID had confidently identified 6, because MusicBrainz models each compilation appearance as its own recording entity and the votes never converge. The AcoustID key is supplied by the operator (`$DUB_ACOUSTID_KEY` or `--key`) and is never stored in the repo. Committed rips also now carry **BPM and INITIALKEY (Camelot)** in their Vorbis comments — the commit path already pre-analysed every segment, but the numbers reached only Dub's catalog, so a FLAC moved to Serato or Traktor arrived with no tempo. **Discogs enrichment** (reached through MusicBrainz's curated link rather than a fuzzy search) and the **app wiring** — `Identify` / `Use these` in the rip review panel, on a background worker polled like the commit job — have since landed, and the Discogs personal access token now lives in the **login Keychain** rather than the preferences plist (R-42; the AcoustID *application* key stays in `UserDefaults`, since it identifies Dub rather than the user and Dub's source is public). What remains of M26c is the tie-break for same-artist pressings: a foreign-language pressing credited to the same artist ties with the domestic one all the way down, and the artist consensus cannot separate them — a script-coherence heuristic could. Deliberate UI gaps are tracked in `UI-BACKLOG.md` §6. Milestone breakdown in §12.2.
...
**Load never blocks playback (M10.5v, streaming head since the M16 load-latency round).** `load_track` returns to the caller the moment the audio thread has the new `Arc<Track>` — a **head decode** (~4 s of audio, O(1) in track length: ~70 ms for a 4-minute tune, ~1.1 s for a 70-minute mixtape, the latter dominated by the full-length buffer allocation) plus a sub-millisecond engine-state swap. The tail streams in on a per-deck `dub-decode-{idx}` thread behind an atomic watermark at hundreds of times realtime (§4.4 "ready to play before decode finishes" made real; pre-streaming, the whole file decoded synchronously — ~12 s for a 70-minute mixtape before the deck could play). Playback reads only RAM: a position past the watermark renders silence and self-heals as the decode frontier passes it; forward/backward symmetry is untouched because both directions read the same buffer, and streaming vs one-shot decodes are pinned byte-identical by test. Offline peaks and `analyze_beat_grid` move to a detached `std::thread::spawn` inside `dub-ffi`, which installs its results back through `Arc<Mutex<EngineState>>` and bumps the per-deck `peak_generation_seq`; the uncached compute paths (no sidecar / no library grid) park until the tail decode lands, while cached loads never wait. The Apple shell calls `loadTrack` from a `Task.detached`, then drops back into the 30 Hz position poll; the waveform appears within ~30 ms of swap, the deck-header BPM within ~300 ms. The user can press `Space` immediately after the load completes — playback is *never* gated on analysis. Cancellation against a back-to-back load on the same deck is by `Arc::ptr_eq`: if a newer track has displaced the in-flight `Arc<Track>` in `running.file_tracks[idx]` by the time the worker thread finishes, the stale peaks / grid are dropped on the worker's stack. **Performance budget**: a 4-minute track must reach a fully populated BPM column in ≤ 1 s on M-series silicon end-to-end; before M10.5v the `Vec::drain(..HOP_SIZE)` in `dub-spectral::SpectralFrameStream::process` made this 38 s for a clean synthetic track (O(N²) collapse on the offline path), which M10.5v fixed with a read cursor + amortised compaction (now O(N)).
...
**Timecode decks — implemented 2026-09-23.** M14 shipped key lock for internal playback only: the engine refused to engage whenever the M6 absolute advance was driving the deck, which is every timecode deck, so on the rig the LOCK lit and the pitch rode the fader. The obstacle was real — the engaged path reads the track through its own cursor (leading the playhead by the stretcher's latency), and the M6 re-pin moves the playhead to the groove at the end of every block without it, so the stretched audio would have drifted off the needle. The re-pin now carries its per-block correction into the stretcher's cursor; a correction too large to be a platter's wobble (> 256 frames) is a discontinuity and drops to bypass, and the settle timer re-primes cleanly. A scratch bypasses through the existing slew rule. Pinned by `key_lock_holds_pitch_on_a_timecode_deck`, `timecode_key_lock_stays_with_the_groove` (which fails with the correction removed) and `a_scratch_bypasses_timecode_key_lock`.
...
- **Manual Loop In / Loop Out** (M13) is the escape hatch from the beat-length grab, not a second loop UX. The grab needs a beat grid; IN / OUT do not, so they are the only way to loop a passage on a track the analyser could not grid, or one whose grid disagrees with the bar the DJ actually wants. IN arms the start at the playhead, OUT closes the region and starts it repeating on the press; both snap to the grid under the same preference hot cues use, so a gridded track still lands clean. ✕ exits a running loop or disarms a half-set one. A manual region lights no length pad — the beat-length row still shows only what it means.
- **No loop relocation.** Still out of scope: "move the loop while it's running" is a controller-DJ idiom we are not building. Re-grab instead.
- **Loops work correctly under timecode** (acceptance §14 #8). The loop is in-engine, not driven by needle position, so the engine owns the looped region without fighting the platter: the decoded platter rate drives the intra-block velocity, the per-sample wrap confines the position, and the M6 absolute re-pin *and* the groove-continuity heal are both suspended while a loop is engaged — the groove climbs monotonically while the playhead wraps in place, so a drift monitor left running would read ever-growing "drift" and slew the playhead forward against the wrap. The drift anchor is re-established on loop exit.
- **A loop puts the deck in relative mode**, and exiting it leaves the deck where the loop left it — playback simply continues from the exit position, as every DJ application does. Dub does *not* jump the playhead to where the record has meanwhile reached (Traktor's Flux mode, Serato's Censor): the record is no longer the position reference once a loop has remapped the deck, and re-establishing it is a re-cue, not an automatic snap.
...
**Hot cues are a v1 performance feature** — fixed trigger points the DJ drops onto a track and fires live for beat-juggling, finger-drumming, and re-triggering a phrase or a drop. They live on the eight CUE pads, fired from **whatever keys the DJ maps to them** (map mode, M18 — there is no default keymap for anything played: the number row was pre-assigned until 2026-09-15), a small pad controller next to the turntables, or by **clicking the pad** (Shift / ⇧-click clears). A hot-cue press is a momentary trigger, not a continuous performance gesture, so the mouse is fine for it (§1); the keys and controller are there for DJs who keep their hands off the trackpad mid-set.
...
- **The file's own key tag.** ID3 `TKEY`, Vorbis `INITIALKEY` and the iTunes `initialkey` atom are read at import (matched by name — symphonia has no standard key for them, and until 2026-09 they were not read at all, so a keyed collection showed blank KEYs until Dub's analyser ran). The verbatim string lands in `track_metadata_source(source='id3', key=…)` for the per-source column, and is also promoted to `track_keys(source='id3')` in Camelot via `dub-library::key_notation`, which accepts Camelot (`8A`), Open Key (`1m`) and musical (`Ebm`, `A minor`) spellings and refuses anything else. The tag ranks above `auto` (an import beats an estimate, §8.3) and below a Serato / Traktor / rekordbox import. We never overwrite the audio file, so whatever a tool wrote round-trips on export untouched.
...
#### 8.4.1 Collection membership

The catalog distinguishes **tracks Dub knows about** from **tracks in the user's collection**. Enabling an external source (Serato, Traktor, rekordbox, iTunes) mirrors that app's *entire* library for **browsing** — its playlist tree plus an "all tracks from this source" node — but those tracks are not, by that act alone, in the user's collection. A foreign app's library is a crate the DJ flips through, not Dub's system of record for what they own.

A track joins the collection — the **All Tracks** surface (§8.5.1), the global-search scope (§8.5.4), the track count — on one of two deliberate events:

- **Folder import.** Adding a filesystem folder (§8.4) is the user saying "this is mine." Every file it ingests is a member immediately.
- **Played from a node.** The first time a deck *starts playing* a node track (transport rolls — `history_play_started`, not a paused load), it is promoted into the collection. Loading a track onto a paused deck to audition it does not promote it; the DJ has to actually play it.

Mechanically this is the `tracks.in_collection` flag (schema v7). External-source scans still mint one `tracks` row per file — the playlist mirror (§8.5.1) and the shared-identity dedupe (§8.1) both key off `tracks.id`, and a node track must resolve to a real row to be loadable — but those rows stay `in_collection = 0` (browse-only) until promoted. The per-source nodes ignore the flag; "All Tracks" filters on it. On upgrade, existing rows backfill from the same two signals (folder-imported, i.e. carrying an `id3`/`filename` metadata row, or ever-played).

**Promotion does not re-analyze.** A promoted node track keeps its imported beatgrid / key as authoritative (the §8.3 priority chain); the lazy first-load analysis (§8.4) only fills in what the source did not provide — Dub's waveform sidecar, the Chromaprint fingerprint, LUFS / auto-gain. We never recompute a grid the source already supplied.
...
- **All Tracks** — the user's **collection**: folder-imported tracks plus any external-source track the DJ has played (§8.4.1). *Not* every mirrored row — enabling a source does not dump its library here; those tracks live under their source node until played.
...
- **Modern, dark, calm.** Not Las-Vegas neon, not skeuomorphic decks. Something closer to Logic Pro / Ableton 12 than to rekordbox 7. Dark is the default and the only theme in v1.0; **Light** (daytime) and **Kingston** (olive / military) follow post-1.0 as M27 (§12.2), which is why every colour lives in `DubColor` and not in a view. The one deliberate exception to "no skeuomorphism" is the dub gear — the siren box and the DUB FX rack draw the 70s hardware they emulate (§6.3); the surface around them stays calm.
...
- `BPM XX.X` — the tempo the record is **actually playing at**: the `dub-bpm` grid BPM scaled by the platter's pitch, so it is the number you beatmatch against and the one §9.4 means by "match the numbers". It follows the *fader*, not the hand: the scaling pitch is held while the deck is scratched (the smoothed rate runs past ±100 % there, which would read as a nonsense BPM and jump the row), so the number changes when you move the pitch and stays put when you cut. Tentative readings shown italicized; falls back to the nominal BPM when the deck isn't being driven.
- `KEY` — `—` until v1.x key-detection ships (or M11-imported library metadata, whichever lands first).
- `LOCK` — M14 key lock, on/off by tap, at the end of the readout row the pitch it holds is on (2026-09-22; before that the control existed but was mounted nowhere, so key lock was unreachable on the performance surface). The button lights from the DJ's switch; its dot reports the *engine's* state — dim while the platter sits at unity or a loop has the stretcher bypassed, solid once a pitch is being held. Bindable in map mode (`keylock.a` / `keylock.b`). Hidden in Prep, which has no platter; the readouts live in the deck column there (§9.2), and `DeckReadouts` draws this row on both.
...
- The **overview is an energy map for scanning**, not a detail view: the DJ reads it to see where a breakdown drops out, where the drop hits, and where the outro begins, and scrubs against it to land there. It renders as a **single-sided envelope on a flat baseline** — height grows from the baseline, rekordbox-style — rather than a centre-mirrored shape; the flat-baseline reading makes the structure legible at a glance and integrates cleanly into the chrome. Height is driven by **energy** (broadband RMS weighted over peak), normalised per-track with headroom, so the loud/quiet structure reads at a glance — a breakdown dips, a drop rises — without a loud brick-wall master saturating the whole strip into a flat block. It's drawn as a solid fill with a baseline→contour gradient and a crisp top line. The baseline is the **bottom edge** in Prep's horizontal band and the strip's **outer (window-frame) edge** in the vertical Performance column. (The *zoomed* playing waveform stays centre-mirrored — it is a scratch-detail surface, not an energy map.)
- **The time axis is in *room* seconds, not track seconds.** One drawable pixel is `DubLayout`-independent: `WaveformRenderer.referenceSecsPerPixel × zoom` of the room's clock, on every deck. The renderer lays columns out in peak chunks, and a chunk is 64 samples *of the track*, so the scale has to be corrected for both the platter's pitch and the file's sample rate (`effectiveTimeAxisZoom` folds both into the zoom). Without that correction two decks beatmatched at the same audible tempo draw their beats at *different* spacings — 92/88 = 4.5 % for an 88 BPM record pitched up to 92, aligned at the playhead and diverging away from it — and their strips do not scroll at the same pixel speed, which is the premise §9.4 hands the phase meter. A 48 kHz file likewise drew 8.8 % wider than a 44.1 kHz one at the same BPM. Both were live until 2026-09-22 and were found on the rig, not in a test. **The scaling pitch follows the fader, and only the fader** (`PlatterAxisFollower`, in the renderer). A pitch sweep stretches or squeezes the picture as it happens, as Serato's does — the held pitch it used first moved only once the platter was still, so a −8 → +8 sweep scrolled at the old scale and jumped into the new one at the end. Everything else holds the zoom *exactly*: outside the ±50 % band, and any change faster than a hand moves a fader (> 40 %/s over 100 ms — STOP, START, a scratch, even a slow in-band drag). The follower runs 80 ms behind the platter, longer than those take to show their speed, so none of their frames reach the axis, and it rejoins only after 0.35 s of calm.
...
| **M12-lexicon** | **Lexicon path documented** — ✅ **shipped** | No code: `LIBRARY-FORMATS.md` documents the Lexicon → rekordbox.xml → Dub path (Lexicon exports; we read the export). | 0.5 day |
...
| **M18** | **Polish + Alpha** | Calibration UX, preferences, ~~key remapping~~ (the keyboard lane of map mode ✅ shipped 2026-09-15 — MAP on the status strip, click a control, press the key; MIDI lane later), dark-mode polish, and manual rig checklist are ready for 3–5 trusted DJs. Includes the deferred M16 fine-tuning: siren sound polish (GS1 shots / DS01E tones / SN76477 bank) and the siren Expert panel's home on the box (`UI-BACKLOG.md` §5 F-36 / F-37). | 2–3 weeks |
...
| **M26** | **Vinyl rip (record real records into the library)** | Record one side on the DJ rig (`dub rip` today, Prep-mode review UI later), split it into tracks, encode FLAC 24-bit + Vorbis tags, and the tracks appear in the library **pre-analyzed** (fingerprint, beat grid, key, LUFS, waveform) — fully offline, with the whole side archived losslessly. Spec in §5.2.7. Sub-milestones: **M26a ✅ shipped** capture + manual split + encode + import; **M26b ✅ shipped** auto gap detection, needle-drop auto-start / silence auto-stop, both-end side trim, session recovery, re-split from archive, review UI + Real Records browser node; **M26c ✅ shipped** recognition (AcoustID naming, opt-in MusicBrainz release tracklist, Discogs enrichment via the `dub-recognize` leaf crate, `Identify` in the review panel, the Discogs token in the login Keychain, sample-lineage link-out); plus the Swift review UI (draggable split markers). Synergy with M21: ripped records seed the library that Thru-mode fingerprint recognition matches against. |
| **M27** | **UI themes — Light and Kingston** (post-1.0, added 2026-09-16) | Two themes beside the default dark surface, chosen in Preferences: **Light** for daytime DJing — a sunlit booth, a garden party, a record fair — where the dark surface is a mirror; and **Kingston** — greenish olive military tones, the sound-system truck, the khaki and brass of the gear the dub instruments already emulate. Demo: switch theme in Preferences, and every surface, pad, meter and waveform lane resolves through `DubColor` with no hard-coded hex anywhere in a view; the Swift snapshot suites run once per theme; the waveform palette and the deck tints keep their function (§9.1 "color = function") in all three. The dark default is untouched. |
