...
| **M11a** | **Library schema + path-by-volume-UUID** | ✅ shipped — `dub-library` crate stood up with the full §8.2 v1 schema (16 tables + FTS5 virtual table + sync triggers + partial unique index for one-active-grid-per-track), migration runner with schema-version 1 baseline and `SchemaTooNew` write guard, WAL + foreign-keys PRAGMAs on every open, macOS volume UUID discovery via `getattrlist(2)` (`ATTR_VOL_UUID`), `Library::open_default` resolving to `~/Library/Application Support/Dub/library.sqlite`, idempotent `upsert_volume` for re-mount handling. `docs/LIBRARY-SCHEMA.md` published as the public API contract per §8.7. The "M10.5j sidecar key migrated from path-hash to canonical fingerprint" sub-deliverable is **deferred to M10.5j's own implementation** — the sidecar cache itself is not yet built (M10.5j is *planned* in the table above), and the new schema's `analysis_cache.waveform_sidecar_path` column reserves the right shape for it. 16 new tests, all green; full workspace at 588 tests, clippy clean. → [SHIPPED §M11a](SHIPPED.md#m11a). | 2 days |
| **M11b** | **Canonical fingerprint + version-aware dedupe** | ✅ shipped — `dub-fingerprint` stood up on `rusty-chromaprint` (pure-Rust algorithm-2 Chromaprint; see §10.2 for the LGPL-2.1 pivot rationale, parallels `dub-bpm` over aubio): `Fingerprint::compute_from_f32` / `compute`, `to_blob` / `from_blob` round-trip for the `fingerprints.chromaprint_blob` column, sliding-window `similarity()` with ±30-item alignment for encoder start-offset tolerance. §8.1 version-token parser in `dub-library::version_tokens` recognises 22 canonical tokens (`clean / dirty / explicit / instrumental / acapella / radio / edit / extended / club / dub / vip / remix / remaster / mono / stereo / intro / outro / short / long / 7in / 12in / lp`) with bracket-only and trailing-dash word-boundary recognition; false-positive guards for "Clean Bandit", "Radiohead", "Nine Inch Nails", "Dirty Vegas". `dub-library::dedupe::decide` enforces all three §8.1 conditions (Chromaprint similarity ≥ 0.98, duration delta < 200 ms, no version-token mismatch). `Library::upsert_fingerprint` + `load_fingerprint` write into the M11a `fingerprints` table; `analysis_cache` rows reference the same `fingerprint_id` per §8.4. 38 new tests; full workspace at 626 tests, clippy clean. → [SHIPPED §M11b](SHIPPED.md#m11b). | 2 days |
| **M11c** | **Filesystem importer + filename parser** | ✅ shipped — `dub-library::import_folder` walks a folder (recursive, deterministic alphabetical order via `walkdir::WalkDir::sort_by_file_name`, audio-extension filter), decodes via `dub-io::Track::load_from_path`, computes the canonical Chromaprint fingerprint via `dub-fingerprint::Fingerprint::compute_from_f32`, runs §8.1 dedupe against `find_fingerprint_neighbours(duration_ms, 200 ms)` neighbours and writes the result. `dub-library::filename_parser` recognises the §8.4 patterns (`ARTIST - TITLE.ext`, `ARTIST - TITLE (VERSION).ext`, `ARTIST_-_TITLE_(VERSION)_[YEAR].ext`, `[LABEL CAT#] ARTIST - TITLE.ext`) with separator-tolerance for underscored variants and trailing-dash version segments; `is_junk_title` flags `Track 01` / `Unknown` / `Untitled` / blogspot-noise titles so the importer falls through to the filename source per §8.4. `dub-io::TrackMetadata` extended to carry the §8.2 `track_metadata_source` field set (`genre`, `comment`, `composer`, `year`, `track_number`, `bpm`, `key`, `gain_db`); container-tag mapping via `symphonia::core::meta::StandardTagKey`. New `Library` write paths: `find_fingerprint_neighbours` (duration-windowed first pass), `insert_track`, `upsert_track_file` (idempotent on `(volume_uuid, relative_path)`), `find_track_file_owner` (idempotent re-import path), `find_track_owner_by_fingerprint_id` (dedupe → display-string assembly via COALESCE over the per-source rows), `upsert_metadata_source` (idempotent on `(track_id, source)`, with NULL-not-empty-string normalisation). macOS firmlink correctness: `volumes::discover_for_path` now normalises the `/System/Volumes/Data` mount-point to `/` so user-visible paths (`/Users/...`, `/var/...`, `/private/var/...`) all share the boot-volume identity and the `track_files.relative_path` prefix-strip works. 22 new tests cover every documented filename pattern, junk-pattern recognition with false-positive guards ("23" / "99 Problems" / "Track 1 (Live)" / "Donuts" stay non-junk), fresh-import (1 track → 1 tracks row, 1 track_files row, 2 metadata rows), idempotent re-import (refresh-not-duplicate), identical-content merge (2 files, 1 track, 2 track_files), clean-vs-dirty sibling registration (`duplicate_link_track_id` populated), non-audio file skip, missing-root error. Workspace test count 626 → 648. Deferred to follow-up sub-milestones: `analysis_cache` LUFS / waveform / `has_active_grid` write (lands with M10.5j's analysis pipeline rebuild on the new schema), `track_beatgrids(source='auto')` cross-validation row (needs `dub-bpm::analyze_bpm` wired into the importer; trivial follow-up). Background/parallel scanning stays single-threaded for v1 per the §8.4 "deterministic + easy to reason about" stance. | 2–3 days |
| **M11d** | **Library browser shell** | Replaces M10.5b `FileBrowserView` with the §8.5 surface. Staged across four sub-milestones: **M11d.1** ✅ shipped (DubLibrary UniFFI object + LibraryView with All Tracks + import-folder + FTS5 search + Space/drag load paths preserved). **M11d.2** ✅ shipped (Recently Played wired through `Library::record_load` + deck-load hook in `WaveformAppModel`; sortable columns via SwiftUI `Table` + `TrackSortKey` enum on the FFI). **M11d.3** ✅ shipped (Per-row indicators: loaded-now A/B glyph driven by `DeckState.loadedLibraryTrackId`; potential-duplicate link with click-to-navigate; missing-file glyph via per-volume reachability cache. Grid-disagreement glyph reserved on the BPM column but always-off in v1.0 — gated on the M11c follow-up that wires `dub-bpm::analyze_bpm` into the importer to populate `track_beatgrids(source='auto')`). **M11d.4** ✅ shipped (Schema v2 adds `track_files.is_missing` + `last_checked_at` + a partial index on `is_missing = 1`. `WaveformAppModel.startMissingFilesScanner()` runs a `.background`-priority `Task` that asks the library for the stalest batch of `track_files` rows, probes each absolute path via `FileManager.fileExists`, and writes the verdict back through `mark_file_state` on a 30 s/5 min adaptive cadence (PRD §8.5.5 "rate-limited so it does not trash SSD lifetime"). The LibraryView footer renders a clickable `N tracks missing · Click to relocate` line when `missing_track_count > 0`; the Relocate sheet wraps `NSOpenPanel` and calls a new `DubLibrary.try_relocate_candidate(absolute_path, limit_missing)` FFI per audio file in the supplied directory — the FFI decodes + fingerprints the candidate, scans up to `limit_missing` missing tracks loaded `created_at ASC`, and commits on the first match with Chromaprint similarity ≥ 0.98 **and** duration delta < 200 ms (mirrors PRD §8.1 dedupe). The original `track_files` row is **never** deleted — `relocate_track` inserts a fresh row, so the touring SSD coming back online auto-resurrects the previous path on the next scanner pass. The 5 000-row "All Tracks" cap is unchanged in v1.0; the paging swap to `list_tracks_sorted` is parked for M11e — empirical measurements show the current single-shot path stays under 80 ms for the 5 000-row "All Tracks" case on the M2 Air, well inside the 200 ms interactive budget). Dub Crates / Imported Sources stay as visual placeholders until M11e / v1.x. | 4–5 days total across sub-milestones |
| **M11c.1** ✅ | **Lazy auto-beatgrid + analysis lifecycle** (shipped — see `docs/SHIPPED.md`) | Closes the M11c follow-up gap. Wires `dub-bpm::analyze_bpm` into a new `dub-library::analysis` helper that writes `track_beatgrids(source='auto', is_active=true)` and `analysis_cache(fingerprint_id, …)`. **Lifecycle is lazy, mirroring Serato / Traktor:** filesystem import (M11c) and external import (M11e+) leave tracks unanalyzed; analysis runs on first deck load via a `Task.detached` triggered from `WaveformAppModel.loadTrack` (audio plays immediately, grid fills in within ~1.5–3 s, deck header BPM badge updates on completion); LibraryView right-click → "Analyze Selected" / "Re-analyze Selected" runs a batched progress task in the footer; cache keyed by canonical fingerprint so re-imports and dedupe-merges are free. Browser dims unanalyzed rows + shows "—" in the BPM column (no ID3 BPM fallback at render time — avoids the "row shows 90, deck shows 174" UX bug). M11d.3 grid-disagreement glyph stops being always-off (still requires an imported grid to disagree with — meaningful from M11e onward). | 2 days |
| **M11c.2** | **Key detection (Camelot canonical)** — ✅ shipped | New DSP block in `dub-spectral::key` per §8.3.2 (dedicated 4096-point chroma STFT → fractional-chromaticity binning → 24 Krumhansl-Kessler templates → Camelot map). Schema v3 adds `track_keys` table parallel to `track_beatgrids` (same `(source, is_active, captured_at)` shape + `key_notation` Camelot + `original_notation` for export round-trip) and `analysis_cache.has_active_key`. Same lazy analysis pipeline as M11c.1 — `analyze_track` decodes the file once, runs grid + key, upserts both tables in §8.3 priority order. LibraryView Key column renders active Camelot with click-to-toggle musical notation (`@AppStorage` preference-sticky per §9); relative-major-aware ⚠ glyph wired against `key_disagreement` flag (always `false` in v1.0 until M11e lands a second source). | 2–3 days |
| **M11d.5** | **Manual crates (playlists)** | Lets the user create, name, drag-add, reorder, and delete Dub-native crates. LibraryView sidebar wires the existing `crates` + `crate_tracks` tables (M11a); track rows gain right-click "Add to Crate…" + multi-select drag onto a sidebar crate. Smart Crates stay placeholders (need a query language; deferred). Touches no schema. | 2–3 days |
| **M11d.6** | **Played From / Played Into** | Surfaces the existing `play_history` data the user explicitly asked for: per-track "Played With" rail under the loaded track in the deck header showing the tracks this track most often transitioned to/from (computed from `play_history.from_track_id` / `to_track_id`); deck-history strip showing the last N tracks played on each deck this session. Pure read from `play_history`; no schema changes. | 1–2 days |
| **M11e** | **Serato importer** | Read `_Serato_/database V2`, `_Serato_/Subcrates/*.crate`, ID3 GEOB tags (`Serato Markers_`, `Serato Markers2`, `Serato BeatGrid`, `Serato Autotags`); populate `track_metadata_source(source=serato)`, `track_beatgrids(source=serato)`, `track_keys(source=serato)`, `imported_crates` / `imported_crate_tracks`; imported hot cues + loops stored to `track_cues` / `track_loops` for §8.6 export round-trip (not surfaced in v1 UI per §6.6). Cross-validation against M11c.1 auto-grid and M11c.2 auto-key produces the grid- and key-disagreement indicators. | 4–5 days |
| **M11d.7** | **Customizable browser columns + per-source disagreement view** | Right-click any column header → grouped, checkable column picker per §8.5.3.1. New `LibraryColumnId` UniFFI enum exposes the full column registry (active-priority metadata, per-source verbatim metadata for `id3 / filename / serato / traktor / rekordbox / mixedinkey`, analysis, audio-file, mix-history groups). Dynamic SELECT generation in `Library::list_tracks_extended(columns: &[LibraryColumnId])`: disabled columns cost zero query time; per-source columns add one `LEFT JOIN` of `track_metadata_source` aliased per source. Choices + order + widths persist in `preferences.json` (one global set across all sidebar sources in v1.0). Slotted after M11e so the per-source columns actually have Serato data to surface; the registry is forward-compatible so M12a–d (rekordbox / Traktor / iTunes) light up their respective columns automatically when those importers land. Default visible columns on first launch: Title, Artist, BPM, Key, Length, Comment. | 3–4 days |
...
**Aggregate:** ~ 20–26 weeks of focused work for v1.0, including beta-gated promotion. The library milestones (M11 + M12) expanded from "1 + 1–2 weeks" to ~6 weeks total in the §8 rewrite once the schema, browser UX (including the M11d.7 customizable columns + per-source disagreement view), lazy-analysis pipeline (auto-grid + auto-key), manual crates, mix history surfacing, export, and round-trip discipline were fully scoped; the rest of the schedule is unchanged. The Beta → Stable gap is **deliberately variable**: we ship Stable when the SLOs are met, not on a schedule. This is the load-bearing protection against the "DJ on stage" failure mode.
