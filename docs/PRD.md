...
| **M5.4.3** | **Calibration speed + UX polish** | M5.4.2 ships *correct* per-rig calibration but at industry-trailing speed: live SL3 + Concorde validation showed (a) ~25 s total wall time for the initial calibrate (10 s carrier capture + 5 s lift capture + the two detection waits), and (b) the 3 s startup probe felt longer than 3 s in practice because the wait-for-carrier preamble adds real time on top. **Goal: match Traktor's "drop the needle, hit calibrate, you're done" feel — total ≤ 5 s for first-time calibration, ≤ 1 s for the startup fingerprint probe on a known-good rig.** Concrete plan: (1) **Single-phase calibration** — eliminate the lift step entirely. Lift noise was already deliberately *not* on the fingerprint, and `derive_thresholds` only consumes lift's `amplitude_p95` for the SNR safety check. We can derive `amplitude` purely from `carrier.amplitude_p5 * 0.5` (the value that matched the user's hand-found threshold within 1 %) and keep the SNR check as a soft warning rather than a hard prerequisite. Persist lift stats as zeroes so the JSON schema stays compatible. (2) **Shorter carrier capture** — 3 s of carrier (≈ 564 blocks @ 256 frames @ 48 kHz) gives stable percentiles within < 1 % of the long-run values; pin to 3 s as the new default. (3) **Faster detection threshold** — drop `STABLE_BLOCKS` from 5 to 2 once the criteria are tight enough that 2 consecutive blocks of conf ≥ 0.95 + |rate-1| < 0.05 unambiguously means "carrier present" (validate against captured M5.3/M5.4.1 sessions, expected zero false-positives). (4) **Startup probe accelerator** — skip the wait-for-carrier preamble when the user hasn't dropped the needle yet; instead, run the probe *concurrently* with the timecode-deck spinup (decoder + engine + output stay ready, fingerprint check runs in the background, falls back to saved thresholds if probe times out). (5) **`dub calibrate --quick`** flag opt-in for the faster mode while we tune defaults; promote to default once a few sessions confirm no regression in derived-threshold quality. **Test deltas:** add a fixture-driven test that a 3 s carrier capture vs. 10 s carrier capture from the same SL3 session yield thresholds within 2 % of each other (so the speed reduction is empirical-grounded). | 1–2 days |
