...
- **Internal sample rate:** track device native; never silently resample. Resample at file→engine boundary only when track SR ≠ device SR (using `rubato` SincFixedOut).
...
| **M2** | **RT discipline + soak harness** | rt-audit green under stress; 1-hour playback with no xruns at 64-sample buffer; **soak test harness in CI runs nightly**; **first parser fuzz target wired up** (ID3 reader). | 3–5 days |
| **M3** | **Disk streaming** | Plays MP3/FLAC/AIFF, sample-accurate seek, backward scratching from internal-mode scrub bar. | 4–6 days |
| **M4** | **Two decks + debug mixer** | Both decks playing through their own output pairs, debug mixer toggles them. | 2–3 days |
| **M5** | **Timecode v1 (Serato)** | Serato CV02 controls deck position. Scope visible. Stickiness works. | 1–2 weeks |
