...
| **M5.4.4** | **Multi-cartridge profiles** | Today the calibration JSON is keyed by `(device_name, format)` — one file per soundcard + format. M5.4.3's faster startup makes "swap cartridge → recalibrate" cheap enough that auto-recalibration covers most users, but vinyl enthusiasts with multiple cartridge bodies (Concorde Pro for sets, Nightclub for backspin abuse, AT-XP3 for archival) would benefit from named profiles. Schema bump: `~/.dub/calibration/<device_key>/<profile_name>.json`; CLI `dub calibrate --name nightclub`; on startup, fingerprint-match across all profiles for the device, pick the closest, fall back to auto-recalibrate if none matches. | 1–2 days |
