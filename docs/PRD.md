...
| **M5.4** | **Calibration + scope** | The lift policy (amplitude gate + confidence hysteresis + sticky window) lives in M5.3 because real-hardware iteration demanded it; M5.4 adds the per-cartridge **calibration UX** (measure typical engage/disengage/amplitude levels for the user's specific cartridge + preamp + interface and store them per device), and a simple **TUI scope**: Lissajous of input `(L, R)` + decoded rate / position / confidence / amplitude bars. | 2–3 days |
