...
| **M11d** | **Library browser shell** | Replaces M10.5b `FileBrowserView` with the §8.5 surface. Staged across four sub-milestones: **M11d.1** ✅ shipped (DubLibrary UniFFI object + LibraryView with All Tracks + import-folder + FTS5 search + Space/drag load paths preserved). **M11d.2** ✅ shipped (Recently Played wired through `Library::record_load` + deck-load hook in `WaveformAppModel`; sortable columns via SwiftUI `Table` + `TrackSortKey` enum on the FFI; client-side sort for the 5k-row M11d snapshot, FFI `list_tracks_sorted` reserved for M11d.4 paging). **M11d.3** Per-row indicators (loaded-now A/B glyph, grid-disagreement ⚠, potential-duplicate link, missing-file). **M11d.4** Background missing-file scanner + Relocate panel per §8.5.5; paging swap to `list_tracks_sorted`. Dub Crates / Imported Sources stay as visual placeholders until M11e / v1.x. | 4–5 days total across sub-milestones |
