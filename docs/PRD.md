...
| **M5.5.2** | **External-mixer 4-channel output routing** | Use the M5.5.1 primitive end-to-end: `OutputOptions { sample_rate, buffer_frames, channels, channel_map }` + `AudioOutput::start_with_options(engine, opts, deck_routing)`. Known-device auto-configuration table maps recognised interfaces by name to their canonical routing — **Serato SL3**: deck A out ch 3+4, deck B out ch 5+6, aux ch 1+2 (matches the SL3's per-deck wiring inside the box, not a `2N+1/2N+2` formula); **Traktor Audio 6**: deck A out ch 1+2, deck B out ch 3+4. Unknown devices require explicit `--deck-a-out-ch` / `--deck-b-out-ch` flags (1-based) or refuse to start with a clear error. `--internal-mixer` is a debug-only escape hatch that forces the M4 path (used during dev-on-laptop where there's no external interface). When no interface is detected at all, the app enters "preparation mode" (browse / import / no live DJing playback) — matches Serato's behaviour and the user's "no mouse DJ ever" rule. CLI: `--output-channels N`, `--device-profile <name>` (override auto-detect), `--internal-mixer`. Live SL3 validation: deck A → SL3 out ch 3+4 → physical mixer's deck-A line in. | 2–3 days |
