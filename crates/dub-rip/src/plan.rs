...
/// Validate a set of split boundaries (in frames) against a recording
/// of `total_frames` frames. Boundaries are the frame where the
...
#[must_use]
pub fn segments(boundaries: &[u64], total_frames: u64) -> Vec<Range<u64>> {
...
mod tests {
    ...
    fn empty_boundary_list_is_one_whole_side_segment() {
        validate_boundaries(&[], secs(60), SR, MIN).unwrap();
        let segs = segments(&[], secs(60));
    ...
    fn three_track_side_validates_and_derives_ranges() {
        ...
        validate_boundaries(&bounds, secs(600), SR, MIN).unwrap();
        let segs = segments(&bounds, secs(600));
        assert_eq!(
            segs,
    ...
    fn rejects_boundary_at_zero_or_past_end() {
        assert!(matches!(
            validate_boundaries(&[0], secs(60), SR, MIN),
            Err(SplitError::OutOfRange { .. })
        ));
        assert!(matches!(
            validate_boundaries(&[secs(60)], secs(60), SR, MIN),
            Err(SplitError::OutOfRange { .. })
        ));
    }
    ...
    fn rejects_non_increasing_boundaries() {
        let err = validate_boundaries(&[secs(30), secs(30)], secs(90), SR, MIN).unwrap_err();
    ...
    fn rejects_too_short_first_middle_and_tail_segments() {
        // First segment 2 s.
        assert!(matches!(
            validate_boundaries(&[secs(2)], secs(60), SR, MIN),
            Err(SplitError::SegmentTooShort { index: 0, .. })
        ));
        // Middle segment 3 s.
        assert!(matches!(
            validate_boundaries(&[secs(10), secs(13)], secs(60), SR, MIN),
            Err(SplitError::SegmentTooShort { index: 1, .. })
        ));
        // Tail segment 1 s.
        assert!(matches!(
            validate_boundaries(&[secs(59)], secs(60), SR, MIN),
            Err(SplitError::SegmentTooShort { index: 1, .. })
        ));
    }
    ...
    proptest! {
        ...
        fn accepted_boundaries_tile_the_recording(
            ...
            bounds.dedup();
            if validate_boundaries(&bounds, total, SR, MIN).is_ok() {
                let segs = segments(&bounds, total);
